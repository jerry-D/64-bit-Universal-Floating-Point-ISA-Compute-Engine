...
//(* ram_style = "distributed" *) reg    [DATA_WIDTH-1:0] twoportRAM[(2**ADDRS_WIDTH)-1:0];
(* ram_style = "block" *)reg    [4:0] twoportRAM[32767:0];
...
always @(posedge CLK) begin
    if (wren) twoportRAM[wraddrs] <= wrdata;
end

always @(posedge CLK) begin
    if (rden) rddata <= twoportRAM[rdaddrs];
end
