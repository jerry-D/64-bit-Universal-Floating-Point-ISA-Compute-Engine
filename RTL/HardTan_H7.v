...
always @(posedge CLK) 
    if ({X, GRSin}  > {16'hbf00, 3'b000} ) begin
        {R, GRSout} <= {16'hbf00, 3'b000};  //-1: if X < -1
        {D, GRSoutD} <= {16'h0000, 3'b000}; // derivative  is 0
    end
    else if (~|X[14:0] || (X[14:9]==6'b011111)) begin
        {R, GRSout} <= {X, GRSin};  // X: if -1 <= X <= 1
        {D, GRSoutD} <= {16'h3F00, 3'b000}; // derivative  is 1
    end    
    else begin
        {R, GRSout} <= {16'h3f00, 3'b000};  //1: if X > 1
        {D, GRSoutD} <= {16'h0000, 3'b000}; // derivative  is 0        
    end    
